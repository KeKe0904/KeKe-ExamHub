...
KeKe ExamHub 是一个面向学校的考试信息展示与管理平台，替代传统手写考试信息方式。提供前台考试信息展示、教室端一体机大屏、监考模式全屏显示、后台考试管理、公告发布、教学楼与教室调度、系统设置等完整功能。
...
- **教室端注册** — 使用注册码注册，选择教学楼，填写教室号，管理员审核后生效
- **教室端大屏** — 教室一体机大屏展示本教室被分配的考试信息，30 秒自动刷新
- **监考模式页面** — 全屏考试倒计时 + 进度条，最后 5 分钟红色预警，支持全屏 API
- **教室分配** — 考试按需分配教室，教室端仅显示被分配到的考试

### 后台管理

- **仪表盘** — 考试数据概览统计
- **考试管理** — 发布、编辑、删除考试，支持分配教室
- **教学楼管理** — 创建、编辑、删除教学楼（教室注册时选择）
- **注册码管理** — 批量生成注册码（XXXX-XXXX 格式），按状态筛选
- **教室端账号管理** — 审核注册申请（通过/驳回），按状态筛选，实时待审核计数
- **公告管理** — 发布、编辑、删除公告
- **环境管理** — 一键检测运行环境组件版本（Node.js / npm / PM2 / Nginx / MariaDB / Git 等），支持分别更新各组件和重装前后端环境（不含数据库），实时日志反馈
...
│   │   ├── middleware/
│   │   │   └── auth.ts               # JWT 双角色认证中间件
│   │   ├── migrations/
│   │   │   ├── init.sql              # 表结构 + 示例数据
│   │   │   └── run.ts                # 命令行迁移工具
│   │   ├── routes/
│   │   │   ├── auth.ts               # 认证（登录/修改密码）
│   │   │   ├── exams.ts              # 考试管理 CRUD + 教室分配
...
1. **创建表结构** — admins / exams / announcements / settings / buildings / registration_codes / classrooms / exam_classrooms 八张表
...
| `buildings` | 教学楼 | id, name, created_at |
| `registration_codes` | 注册码 | id, code, is_used, used_by_classroom_id, created_at |
| `classrooms` | 教室端账号 | id, room_number, building_name, status, registration_code_id |
| `exam_classrooms` | 考试-教室关联 | exam_id, classroom_id (联合主键) |
...
管理员可在后台「服务器环境」页面直接操作环境重装，无需 SSH 登录服务器：

| 操作 | 说明 | 影响 |
|------|------|------|
| 重装前端 | 清除前端 `node_modules` → 重新安装 → 重新构建 | 前端短暂不可用，API 不受影响 |
| 重装后端 | 清除后端 `node_modules` → 重新安装 → 重新编译 → 自动重启 | API 中断约 10 秒后自动恢复 |
| 完整重装 | 同时清理前后端依赖，依次安装/编译/构建 → 自动重启 | 服务中断约 30 秒，数据库不受影响 |

> 所有重装操作均保留 `.env` 配置文件和数据库数据，不会丢失任何考试/公告/教室信息。
...
mysql -u examhub -p examhub -e "
  DROP TABLE IF EXISTS admins, exams, announcements, settings,
    buildings, registration_codes, classrooms, exam_classrooms;
"
...
| GET | `/api/buildings` | 获取教学楼列表（教室注册用） |

### 教室端接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/classroom/register` | 教室端注册 |
| POST | `/api/classroom/login` | 教室端登录 |
| GET | `/api/classroom/verify` | 验证教室端 JWT Token |
| GET | `/api/classroom/exams` | 获取本教室被分配的考试 |
| GET | `/api/classroom/profile` | 获取本教室信息 |
...
| GET | `/api/exams/stats/overview` | 考试统计数据 |
| GET | `/api/exams/:id/classrooms` | 获取考试已分配的教室 |
| POST | `/api/exams/:id/classrooms` | 批量分配教室 |
| DELETE | `/api/exams/:id/classrooms/:classroomId` | 取消教室分配 |
| GET | `/api/exams/classrooms/available` | 获取可分配教室列表 |
| POST | `/api/announcements` | 创建公告 |
| PUT | `/api/announcements/:id` | 更新公告 |
| DELETE | `/api/announcements/:id` | 删除公告 |
| GET | `/api/announcements/admin/all` | 获取全部公告（含未发布） |
| GET | `/api/settings` | 获取系统设置（管理） |
| PUT | `/api/settings` | 更新系统设置 |
| GET | `/api/settings/profile` | 获取管理员个人信息 |
| PUT | `/api/settings/password` | 修改密码 |
| PUT | `/api/settings/avatar` | 更换头像 |
| PUT | `/api/school-info` | 更新学校信息 |
| POST | `/api/buildings` | 创建教学楼 |
| PUT | `/api/buildings/:id` | 更新教学楼 |
| DELETE | `/api/buildings/:id` | 删除教学楼 |
| GET | `/api/registration-codes` | 获取注册码列表 |
| POST | `/api/registration-codes` | 生成注册码 |
| DELETE | `/api/registration-codes/:id` | 删除注册码 |
| GET | `/api/classrooms` | 获取教室端账号列表 |
| PUT | `/api/classrooms/:id/approve` | 审核通过 |
| PUT | `/api/classrooms/:id/reject` | 驳回申请 |
| DELETE | `/api/classrooms/:id` | 删除教室端账号 |
| GET | `/api/classrooms/pending/count` | 获取待审核数量 |
| GET | `/api/environment` | 获取服务器环境信息 |
| POST | `/api/environment/reinstall` | 重装环境（frontend / backend / all） |
