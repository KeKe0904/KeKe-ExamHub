...
| 步骤 | 内容 | 说明 |
|------|------|------|
| 1/6 | 环境检测与安装 | 自动安装 Node.js 22 LTS / Nginx / MariaDB / PM2（已安装则跳过） |
| 2/6 | 构建后端 | `cd api && npm install && npm run build` |
| 3/6 | 构建前端 | `npm install && npm run build` |
| 4/6 | 配置 PM2 | 生成 `ecosystem.config.cjs`，cwd 指向 api 目录，启动后端进程 |
| 5/6 | 配置 Nginx | gzip 压缩 + 静态资源缓存 + SPA 路由 + API 反向代理 |
| 6/6 | 打印引导信息 | 提示创建数据库用户 + 打开安装向导地址 |

**脚本特性：**

- **幂等可重复执行** — 已安装的组件自动跳过，可安全重复运行
- **不做数据库初始化** — 数据库和用户需手动创建，表结构由安装向导创建
- **构建顺序** — 先后端再前端，避免交叉依赖问题
- **cwd 明确** — PM2 配置指定 cwd 为 api 目录，确保 .env 和 .setup-complete 位置确定
...
| Node.js 安装失败 | 网络问题 | 重试或手动安装：`curl -fsSL https://deb.nodesource.com/setup_22.x \| bash - && apt-get install -y nodejs` |
