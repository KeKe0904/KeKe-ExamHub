    ...
    async (request, reply) => {
      try {
        ...
        try {
          ...
          await conn.execute("DELETE FROM classrooms WHERE id = ?", [id]);
          // 释放注册码
          if (regCodeId) {
            await conn.execute(
          ...
          // 清理教室关联数据(部分表未设置外键级联,需手动删除)
          await conn.execute(
            "DELETE FROM classroom_login_logs WHERE classroom_id = ?",
            [id]
          );
          await conn.execute(
            "DELETE FROM classroom_trusted_ips WHERE classroom_id = ?",
            [id]
          );
          await conn.execute(
            "DELETE FROM exam_classrooms WHERE classroom_id = ?",
            [id]
          );
          await conn.commit();
        } catch (err) {
