...
export default function Hero({ stats, compact }: HeroProps) {
  const schoolName = useSchoolName();
  const displayName = schoolName || "KeKe ExamHub";
  return (
    <section className={`relative overflow-hidden bg-zinc-100 dark:bg-black ${compact ? "rounded-xl" : ""}`}>
      {/* 装饰几何图形 */}
      {!compact && (
      ...
      <div className={`container relative mx-auto px-4 sm:px-6 lg:px-8 ${compact ? "py-3" : "py-16 lg:py-24"}`}>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h1 className={`font-serif font-bold text-black dark:text-white mb-2 animate-slide-up ${compact ? "text-xl" : "text-4xl sm:text-5xl lg:text-6xl mb-6"}`}>
            让考试信息
            <span className={`${compact ? "ml-2" : "block text-zinc-700 dark:text-zinc-300 mt-2"}`}>一目了然</span>
          </h1>
          ...
          {/* 统计数据卡片 */}
          <div className={`grid grid-cols-3 gap-2 max-w-2xl mx-auto animate-scale-in ${compact ? "" : "gap-4"}`}>
            <StatCard
...
function StatCard({ icon, value, label, color }: StatCardProps) {
  ...
  return (
    <div className="bg-zinc-50 dark:bg-black rounded-lg p-3 border border-zinc-200 dark:border-zinc-600 shadow-sm">
      <div
        className={`w-8 h-8 rounded-lg flex items-center justify-center mb-1.5 border ${colorClasses[color]}`}
      >
        {icon}
      </div>
      <div className="text-xl font-bold font-serif text-black dark:text-white">{value}</div>
