...
function Gauge({ value, size = 120 }: { value: number; size?: number }) {
  ...
  const strokeColor =
    value >= 90 ? "#ef4444" : value >= 70 ? "#f59e0b" : "#000";
...
function MiniBar({ value }: { value: number }) {
  const color =
    value >= 90 ? "bg-red-500" : value >= 70 ? "bg-amber-500" : "bg-black dark:bg-white";
  return (
              ...
              {data.memory.swap && (
                <div className="pt-3 border-t border-zinc-100 dark:border-zinc-900">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-medium ${data.memory.swap.usage >= 80 ? "text-red-500" : data.memory.swap.usage >= 50 ? "text-amber-500" : "text-zinc-400 dark:text-zinc-600"}`}>
                ...
                {data.disks.map((disk: any, idx: number) => (
                  <div key={idx} className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`text-xs font-medium ${disk.usage >= 90 ? "text-red-500" : disk.usage >= 70 ? "text-amber-500" : "text-zinc-400 dark:text-zinc-600"}`}>
                        {disk.usage}%
                      </span>
                    </div>
